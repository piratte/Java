...
public class CodEx {
  ...
  static String zpracujVyraz(String vyraz) {
    ...
    for (String atom : split) {
      if (atom.isEmpty())
        continue;
      // bud cislo, nebo cislo s minusem
      try {
        d = new BigDecimal(atom);
  ...
  static BigDecimal vyhodnot(String vyraz) {
    String[] split = vyraz.split("\\s+");
    Queue<BigDecimal> stack = new Queue<BigDecimal>();
    BigDecimal d = new BigDecimal("0.0");
    char op = ' ';
    ...
    for (int i = 1; i < split.length; i++) {
      try {
        ...
        op = split[i].charAt(0);
        try {
          BigDecimal y = stack.popLast();
          BigDecimal x = stack.popLast();
          switch (op) {
            case '+': stack.put(x.add(y)); break;
            case '-': stack.put(x.subtract(y)); break;
            case '*': stack.put(x.multiply(y)); break;
            case '/':
              if (y.compareTo(BigDecimal.ZERO) == 0)
                throw new ArithmeticException("/ by zero"); 
              stack.put(x.divide(y, BigDecimal.ROUND_HALF_UP)); break;
  ...
  static HashMap<String,BigDecimal> varMap = new HashMap<String,BigDecimal>();

  public static void main(String[] argv) {    
    String line;
    String[] lineArr;
    final BigDecimal lastDefVal = new BigDecimal("0.0");

    varMap.put("last",lastDefVal);
    BigDecimal val = lastDefVal;
    String dosazujeme = "";
      ...
      while ((line = input.readLine()) != null) {
        try { 
          ...
          if (lineArr[0].equals("DEF")) {
            ...
            varMap.put("last",lastDefVal);
            continue;
          } else if (lineArr[0].equals("precision")) {
            debug("precision" + lineArr[0] + " " + lineArr[1]);
            int newScale = Integer.parseInt(lineArr[1]);
            prec = newScale;
            // zmenit u vsech BD precision na argument
            for (String key : varMap.keySet()) {
              BigDecimal cur = varMap.get(key);
              cur.setScale(newScale);
              varMap.put(key, cur);
            }
            continue;
            ...
            if (!varMap.containsKey(lineArr[0]))
              varMap.put(lineArr[0], new BigDecimal("0"));
            line = " " + line + " ";
